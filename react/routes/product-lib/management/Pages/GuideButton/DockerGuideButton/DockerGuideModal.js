...
const GuideModal = ({ guideInfo, formatMessage }) => {
  ...
  return (
    <div className="product-lib-docker-guide-modal">
      ...
      <div className="product-lib-docker-guide-modal-description" style={{ height: 'auto', marginBottom: '12px' }}>
        编辑Docker客户端的hosts文件，添加Harbor主机域名和ip地址的映射。
      </div>
      <pre><Button icon="content_copy" onClick={() => handleCopy(info.vimHostCmd)} />{info.vimHostCmd}</pre>
      ...
      <div className="product-lib-docker-guide-modal-description">
        3. docker主机上新增/etc/docker/daemon.json 并增加配置
      </div>
      ...
      <div className="product-lib-docker-guide-modal-description">
        {formatMessage({
          id: `${intlPrefix}.view.docker.buildAndPush.configStep1`,
          defaultMessage: '1. 打开docker客户端，使用harbor用户名密码登陆harbor访问地址',
        })}
      </div>
