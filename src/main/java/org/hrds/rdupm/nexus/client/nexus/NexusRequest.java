...
public class NexusRequest {
    ...
    private String getLocalToken(NexusServer nexusServer) {
        String basicInfo = nexusServer.getUsername() + ":" + nexusServer.getPassword();
        return AUTH_PRE + Base64.getEncoder().encodeToString(basicInfo.getBytes());
    }
...
//        NexusServer nexusServer = new NexusServer(nexusServerConfig.getServerUrl(), nexusServerConfig.getUserName(), nexusServerConfig.getPassword());
        ...
        headers.setContentType(type);
        headers.add(AUTH_HEADER, this.getLocalToken(currentNexusServer));
        ...
        try {
            ...
            throw new CommonException(NexusApiConstants.ErrorMessage.NEXUS_SERVER_ERROR,e);
        }
        this.handleResponseStatus(responseEntity);
        return responseEntity;
    }
