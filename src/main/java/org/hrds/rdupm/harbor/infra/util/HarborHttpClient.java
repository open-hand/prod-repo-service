...
public class HarborHttpClient {
    ...
    public ResponseEntity<String> exchange(HarborConstants.HarborApiEnum apiEnum, Map<String, Object> paramMap, Object body, boolean adminAccountFlag, Object... pathParam) {
        ...
        if (adminAccountFlag || isRoot || HarborConstants.ANONYMOUS.equals(userName)) {
            ...
            if (prodUser == null) {
                prodUser = prodUserRepository.select(ProdUser.FIELD_USER_ID, DetailsHelper.getUserDetails() == null ? 0L : DetailsHelper.getUserDetails().getUserId()).stream().findFirst().orElse(null);
            }
    ...
    public ResponseEntity<String> customExchange(HarborConstants.HarborApiEnum apiEnum, Map<String, Object> paramMap, Object body, Object... pathParam) {
        ...
        try {
            ...
            switch (statusCode) {
                case 401:
                    throw new CommonException("error.harbor.custom.repo.current.user");
                case 500:
                    throw new CommonException("error.harbor.custom.repo.url");
                default:
                    break;
            }
            handleStatusCode(apiEnum, e);
    ...
    private void handleStatusCode(HarborConstants.HarborApiEnum apiEnum, HttpClientErrorException e) {
        ...
        switch (statusCode) {
            case 401:
                throw new CommonException("User need to log in first. or User has no permission to the source project or destination project.");
            case 415:
                throw new CommonException("The Media Type of the request is not supported, it has to be \"application/json\"");
            case 500:
                throw new CommonException("Unexpected internal errors.");
            default:
                break;
        }
        ...
        switch (apiEnum) {
            case CREATE_PROJECT:
                switch (statusCode) {
                    case 400:
                        throw new CommonException("error.harbor.api.create.project.name.valid");
                    case 409:
                        throw new CommonException("Project name already exists.");
                    default:
                        throw new CommonException(e.getMessage());
                }
            case UPDATE_PROJECT:
                switch (statusCode) {
                    case 400:
                        throw new CommonException("Illegal format of provided ID value.");
                    case 403:
                        throw new CommonException("User does not have permission to the project.");
                    case 404:
                        throw new CommonException("Project ID does not exist.");
                    default:
                        throw new CommonException(e.getMessage());
                }
            case DETAIL_PROJECT:
                switch (statusCode) {
                    case 403:
                        throw new CommonException("User does not have permission to the project.");
                    case 404:
                        throw new CommonException("Project does not exist.");
                    default:
                        throw new CommonException(e.getMessage());
                }
            ...
            case CREATE_USER:
                switch (statusCode) {
                    case 400:
                        throw new CommonException("Unsatisfied with constraints of the user creation.");
                    case 403:
                        throw new CommonException("User registration can only be used by admin role user when self-registration is off.");
                    default:
                        throw new CommonException(e.getMessage());
                }
            case COPY_IMAGE_TAG:
                switch (statusCode) {
                    case 400:
                        throw new CommonException("Invalid image values provided.");
                    case 403:
                        throw new CommonException("Forbiden as quota exceeded.");
                    case 404:
                        throw new CommonException("Project or repository not found.");
                    case 409:
                        throw new CommonException("Target tag already exists.");
                    case 412:
                        throw new CommonException("error.harbor.image.tag.copy.412");
                    default:
                        throw new CommonException(e.getMessage());
                }
            ...
            case CREATE_ONE_AUTH:
                switch (statusCode) {
                    case 400:
                        throw new CommonException("Illegal format of project member or project id is invalid, or LDAP DN is invalid.");
                    case 403:
                        throw new CommonException("User in session does not have permission to the project.");
                    case 409:
                        throw new CommonException("A user group with same group name already exist or an LDAP user group with same DN already exist.");
                    default:
                        throw new CommonException(e.getMessage());
                }
            case UPDATE_PROJECT_QUOTA:
                switch (statusCode) {
                    case 400:
                        throw new CommonException("Illegal format of quota update request.");
                    default:
                        throw new CommonException(e.getMessage());
                }
            case DELETE_PROJECT:
                switch (statusCode) {
                    case 400:
                        throw new CommonException("Invalid project id.");
                    case 403:
                        throw new CommonException("User need to log in first.");
                    case 404:
                        throw new CommonException("Project does not exist.");
                    case 412:
                        throw new CommonException("error.delete.project.412");
                    default:
                        throw new CommonException(e.getMessage());
                }
            case GET_ONE_ROBOT:
                switch (statusCode) {
                    case 403:
                        throw new CommonException("User in session does not have permission to the project.");
                    case 404:
                        throw new CommonException("The robot account is not found.");
                    default:
                        throw new CommonException(e.getMessage());
                }
            case DELETE_ROBOT:
            ...
            case CHANGE_PASSWORD:
                switch (statusCode) {
                    case 400:
                        return;//throw new CommonException("the new password can not be same with the old one.");
                    default:
                        throw new CommonException(e.getMessage());
                }
            default:
